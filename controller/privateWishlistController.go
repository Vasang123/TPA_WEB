...
func duplicateItemsIntoWishlist(list []model.WishlistDetail, wishlist model.Wishlist) error {
	db := connect.Connect()
	defer db.Close()
	...
	err := db.Insert(newWishlist)
	// _, err := db.ModelContext(context.Background()).Returning("*").Insert(&newWishlist)
	if err != nil {
		return err
	}

	// Duplicate all items in the list into the new wishlist
	for _, item := range list {
		newItem := &model.WishlistDetail{
			WishlistId: newWishlist.ID,
			ProductId:  item.ProductId,
			Quantity:   item.Quantity,
		}
		err := db.Insert(newItem)
		if err != nil {
			return err
		}
	}
...
func HandleDuplicateItems(w http.ResponseWriter, r *http.Request) {
	var req model.DuplicateRequest

	err := json.NewDecoder(r.Body).Decode(&req)

	fmt.Print("Wishlist :", req.Items, "\n")
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
	err = duplicateItemsIntoWishlist(req.Items, req.Wishlist)
	if err != nil {
		log.Println("Error decoding wishlist payload:", err)
		return
	}
	...
	w.WriteHeader(http.StatusOK)
	w.Write([]byte("Items duplicated successfully"))
}
